...
uint16_t getDistanceToLeftObject()
{
  ...
  uint16_t wheelDistanceUnits = pulseIn(LEFT_SRF05_ECHO_PIN, HIGH, MAX_uS);        // Read in times pulse
                         // Calculate distance from time of pulse
  return(wheelDistanceUnits);
}  
...
uint16_t getDistanceToRightObject()
{
  ...
  uint16_t wheelDistanceUnits = pulseIn(RIGHT_SRF05_ECHO_PIN, HIGH, MAX_uS);        // Read in times pulse
                         // Calculate distance from time of pulse
  return(wheelDistanceUnits);
}
...
uint16_t getDistanceToRearObject()//  get code from 
{
  ...
  uint16_t wheelDistanceUnits = pulseIn(REAR_SRF05_ECHO_PIN, HIGH, MAX_uS);        // Read in times pulse
                         // Calculate distance from time of pulse
  return(wheelDistanceUnits);
}
...
void followWallRight()// until closest wall ends
{
    uint16_t distance;
