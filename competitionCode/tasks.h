...
bool getYellowVoltageDirection()
{
...
bool getBlueCapacitanceDirection()
{
    elapsedMillis elapsed;
    bool direction = RIGHT;
    boolean validDirectionReceived = false;
    pinMode(CAP_TEST_TRIGGER_PIN, OUTPUT);
    digitalWrite(CAP_TEST_TRIGGER_PIN, HIGH);
    delay(10);
    digitalWrite(CAP_TEST_TRIGGER_PIN, LOW);
    while(!validDirectionReceived)
    {
        if(HIGH == digitalRead(EXTERNAL_TEST_A_PIN) && LOW == digitalRead(EXTERNAL_TEST_B_PIN))
        {
            direction = LEFT;
            validDirectionReceived = true;
        }
        else if(LOW == digitalRead(EXTERNAL_TEST_A_PIN) && HIGH == digitalRead(EXTERNAL_TEST_B_PIN))
        {
            direction = RIGHT;
            validDirectionReceived = true;
        }
        ...
        {
            // invalid direction signal so far
...
bool getGreenTemperatureDirection()
{
     elapsedMillis elapsed;
    bool direction = RIGHT;
    boolean validDirectionReceived = false;
    pinMode(TEMP_TEST_TRIGGER_PIN, OUTPUT);
    digitalWrite(TEMP_TEST_TRIGGER_PIN, HIGH);
    delay(10);
    digitalWrite(TEMP_TEST_TRIGGER_PIN, LOW);
    while(!validDirectionReceived)
    {
        if(HIGH == digitalRead(EXTERNAL_TEST_A_PIN) && LOW == digitalRead(EXTERNAL_TEST_B_PIN))
        {
