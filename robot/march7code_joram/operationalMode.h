...
void  turnNorthLoop()
{
  ...
  uint8_t currentPosition = getBearing();
  // how do you determine if the bot is facing south, east, or west; what information is getBearing sending us back?
  int8_t south;
  int8_t east;
  int8_t west;
  ...
  static const int32_t ninetyDegreeTurn = 187;
  static const int32_t threesixtyDegreeTurn = 748;
  static const int32_t oneeightyDegreeTurn = 374;
 ...
 drive4wheelSpeeds(127, -127, 127, -127);
  ...
  if (currentPosition == south)
  {
    while (!(LFstopped && RFstopped && LRstopped && RRstopped))
    {
      if (getEncoderCount(LF) >= startLF+oneeightyDegreeTurn);
      {
        driveWheelSpeed(LF, 0);
        LFstopped = true;
      }
       if (getEncoderCount(RF) <= startRF-oneeightyDegreeTurn);
      {
        driveWheelSpeed(RF, 0);
        RFstopped = true;
      }
       if (getEncoderCount(LR) >= startLR+oneeightyDegreeTurn);
      {
        driveWheelSpeed(LR, 0);
        LRstopped = true;
      }
       if (getEncoderCount(RR) <= startRR-oneeightyDegreeTurn);
      {
        driveWheelSpeed(RR, 0);
        RRstopped = true;
      }
...
{  
  ...
  while (!(LFstopped && RFstopped && LRstopped && RRstopped))
    {
      if (getEncoderCount(LF) >= startLF + 600);
        {
          driveWheelSpeed(LF, 0);
          LFstopped = true;
        }
      if (getEncoderCount(RF) >= startRF + 600);
        {
          driveWheelSpeed(RF, 0);
          RFstopped = true;
        }
      if (getEncoderCount(LR) >= startLR + 600);
        {
          driveWheelSpeed(LR, 0);
          LRstopped = true;
        }
        if (getEncoderCount(RR) >= startRR + 600);
        {
          driveWheelSpeed(RR, 0);
          RRstopped = true;
        }
