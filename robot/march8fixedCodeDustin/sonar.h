...
static const uint16_t MAX_uS = 14800;
...
void displaySonar(uint16_t distanceToFrontObject, uint16_t distanceToLeftObject, uint16_t distanceToRightObject, uint16_t distanceToRearObject)
{
  ...
  lcd.setCursor(0, 8);//lcd.setCsrLC(0, 8);                                        //move cursor to first line, 8th column  
  lcd.print(distanceToFrontObject, DEC);                       // distance to front object in number of wheel degrees
  lcd.setCursor(1, 0);// lcd.setCsrLC(1, 0);                                        //move cursor to second line, 1st column  
  lcd.print(distanceToLeftObject, DEC);                       // distance to left object in number of wheel degrees
  lcd.setCursor(1, 16);// lcd.setCsrLC(1, 16);                                        //move cursor to second line, 16th column  
  lcd.print(distanceToRightObject, DEC);                       // distance to right object in number of wheel degrees
  lcd.setCursor(2, 8);//lcd.setCsrLC(2, 8);                                        //move cursor to third line, 8th column  
...
void calibrateSonar()// start with bot against a wall
{
    ...
    fullStop();
    delay(1000);
    uint16_t pinguS = getTimeToRearObject();
    ...
    Serial.print("speed : ");
    Serial.println(distanceTraveled * 872.665, DEC);
